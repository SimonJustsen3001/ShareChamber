...
const RateForm = observer(
  ({ listId, movieId, movieTitle, personalRating }: Props) => {
    ...
    useEffect(() => {
      setStars(tempStars);
      if (personalRating) setRating(personalRating);
    }, [movieListStore, movieStore]);
        ...
        {({ handleSubmit, isSubmitting, errors, isValid }) => (
          <Form className="form" onSubmit={handleSubmit} autoComplete="off">
            ...
            <ErrorMessage
              name="error"
              render={() => (
                <label className="error-label">{errors.error}</label>
              )}
            />
